...
execute if entity @s[predicate=icewarrior:check_hitbox] unless entity @n[tag=icew.target,distance=..128] run execute on vehicle on target run execute if entity @s[tag=!icew.immune] run function icewarrior:set_target
#auto-target by checking entity around
execute if entity @s[predicate=icewarrior:check_hitbox] unless entity @n[tag=icew.target,distance=..128] run execute if entity @n[type=#icewarrior:aggro_boss,tag=!icew.immune,distance=..24] run execute on vehicle if entity @s[nbt={HurtTime:0s}] unless entity @s[tag=icew.aggro_dmg] run function icewarrior:boss/aggro_boss
