...
execute on vehicle positioned ~ ~-2.5 ~ positioned ^ ^ ^0.5 run execute if function icewarrior:boss/attacks/damage/trident_slash run execute on passengers run scoreboard players add @s icew.combo 1
...
execute on vehicle run launch @s setMotion 0 0.5 0
execute on vehicle at @s facing entity @n[tag=icew.target,distance=..6] eyes run launch @s looking -0.25
