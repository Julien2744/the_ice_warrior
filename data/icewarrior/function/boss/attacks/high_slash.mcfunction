...
execute on vehicle at @n[type=item_display,tag=aj.ice_warrior.root,distance=..4] run execute positioned ~ ~-1.5 ~ positioned ^ ^ ^0.8 if entity @n[tag=icew.target,distance=..1.9] run damage @n[tag=icew.target,distance=..1.9] 14 icewarrior:ice_warrior_attack by @s
