execute positioned ~ ~-1.5 ~ positioned ^ ^ ^0.8 if entity @n[tag=icew.target,distance=..1.9] run scoreboard players add @s icew.combo 1
execute positioned ~ ~-1.5 ~ positioned ^ ^ ^0.8 unless entity @n[tag=icew.target,distance=..1.9] run scoreboard players set @s icew.combo 0
playsound minecraft:entity.wither.shoot hostile @a[distance=..8] ~ ~ ~ 2 1
execute positioned ~ ~-1.5 ~ positioned ^ ^ ^0.8 if entity @n[tag=icew.target,distance=..1.9] run damage @n[tag=icew.target,distance=..1.9] 14 minecraft:mob_attack by @s
execute positioned ~ ~-1.5 ~ positioned ^ ^ ^0.8 if entity @n[tag=icew.target,distance=..1.9] run launch @n[tag=icew.target,distance=..1.9] looking 1